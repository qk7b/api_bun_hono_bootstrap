...
class PostgresAuthenticationService implements AuthenticationService {
    async createUser ({ email, passwordHash }: { email: string; passwordHash: string; }): Promise<string> {
        const [user] = await db`
    ...
    async updateUser ({ id, email, passwordHash, isValidated }: { id: string } & Partial<{ email: string; passwordHash: string; isValidated: boolean; }>): Promise<void> {
        await db`
            UPDATE users u
    ...
    async getUser ({ email, id }: Partial<{ email: string; id: string; }>): Promise<AuthUser | null> {
        const [user] = await db`
    ...
    async updateCode ({ id, voided }: { id: string; voided: boolean; }): Promise<void> {
        await db`
